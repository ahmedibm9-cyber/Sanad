...
begin
  if tg_table_name = 'todos' then return new; end if;
  ...
  if tg_table_name = 'work_items' then
    ...
    ) then raise exception 'Customer must belong to the same company'; end if;
    return new;
  end if;
  ...
  if tg_table_name = 'documents' then
    ...
    ) then raise exception 'Work item must belong to the same company'; end if;
    return new;
  end if;

  if tg_table_name in ('notes', 'report_issues', 'attachments') then
    select company_id into parent_company_id from public.work_items where id = new.work_item_id;
    if parent_company_id is null then raise exception 'Work item not found: %', new.work_item_id; end if;
    if parent_company_id <> new.company_id then raise exception 'Company mismatch with work item'; end if;
    ...
    ) then raise exception 'Document must belong to the same company'; end if;
    return new;
  end if;
  ...
  if tg_table_name in ('material_files', 'material_price_events') then
    ...
    ) then raise exception 'Material must belong to the same company'; end if;
    return new;
  end if;
