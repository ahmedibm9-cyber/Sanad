...
export class MaterialService {
  ...
  async updateMaterial(id: string, input: UpdateMaterialInput, context: RequestContext): Promise<Material> {
    ...
    const existing = await this.getMaterialById(id, context)
    ...
    if (error) {
      if (error.code === 'PGRST116' || error.message?.includes('No rows')) {
        throw new Error('Conflict: Material was modified by another user. Please reload and try again.')
      }
  ...
  async deleteMaterial(id: string, context: RequestContext): Promise<void> {
    ...
    const { error } = await (this.supabase as any)
      ...
      .eq('id', id)
      .eq('company_id', material.company_id)
    ...
    if (trashError) {
      await (this.supabase as any).from('materials').update({ deleted_at: null }).eq('id', id).eq('company_id', material.company_id)
