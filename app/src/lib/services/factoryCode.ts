...
export class FactoryCodeService {
  ...
  async smartMergeWithStaging(
    ...
    // Database functions keep validation and the merge in one authoritative path.
    const { error: validationError } = await (this.supabase as any).rpc('validate_staging_rows', { p_import_id: importId })
    if (validationError) throw handleSupabaseError(validationError)

    const { data: mergeRows, error: mergeError } = await (this.supabase as any)
      .rpc('apply_factory_code_import', { p_import_id: importId })
    if (mergeError) throw handleSupabaseError(mergeError)

    const merge = mergeRows?.[0]
    if (!merge) throw new Error('Factory Code import did not return a committed summary.')

    const { data: committedRows, error: committedRowsError } = await (this.supabase as any)
      .from('factory_code_staging')
      .select('factory_code, factory_name, city, product, status')
      .eq('import_id', importId)
      .in('status', ['inserted', 'updated', 'unchanged'])
      .limit(100)
    if (committedRowsError) throw handleSupabaseError(committedRowsError)
    ...
    return {
      ...
      errors: 0,
      preview: (committedRows || []).map((row: any) => ({
        factory_code: row.factory_code || '', factory_name: row.factory_name || '',
        city: row.city || '', product: row.product || '',
        action: row.status === 'inserted' ? 'add' : row.status === 'updated' ? 'update' : 'unchanged',
      })),
    }
  ...
  /** The Factory Code identifier is required for a safe, repeatable import. */
  static validateImportRows(rows: Array<Record<string, unknown>>): void {
    if (rows.length === 0) {
      throw new Error('The workbook contains no data rows.')
    }

    const columns = Object.keys(rows[0])
    if (!columns.includes('Factory Code') && !columns.includes('factory_code')) {
      throw new Error('The workbook must include a Factory Code column.')
    }
  }
