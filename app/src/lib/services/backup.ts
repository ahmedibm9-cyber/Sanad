...
export class BackupService {
  ...
  async createManualBackup(context: RequestContext): Promise<BackupRecord> {
    ...
    try {
      ...
      // Export actual row data to R2
      await this.exportTableData(backup.id, manifest, context)
  ...
  async restoreBackup(
    ...
    // 3. Import data from R2
    const result = await this.importTableData(backupId, manifest, context)

    appLogger.info('Backup restore completed', {
      backupId,
  ...
  async generateManifest(context: RequestContext): Promise<BackupManifest> {
    ...
    // Compute real SHA-256 checksum of manifest content
    const encoder = new TextEncoder()
    const manifestBytes = encoder.encode(JSON.stringify({ ...manifest, checksum: '' }))
    const hashBuffer = await crypto.subtle.digest('SHA-256', manifestBytes)
    const hashArray = Array.from(new Uint8Array(hashBuffer))
    manifest.checksum = `sha256-${hashArray.map(b => b.toString(16).padStart(2, '0')).join('')}`
  ...
  /**
   * Export actual row data from each table to R2 for real restore capability.
   */
  private async exportTableData(
    ...
    for (const table of COMPANY_TABLES) {
      const { data: rows } = await (this.supabase as any)
        .from(table)
        .select('*')
        .eq('company_id', context.companyId)
      ...
      const encoder = new TextEncoder()
      await uploadToR2(objectKey, encoder.encode(JSON.stringify(rows)), 'application/json', context.companyId)
    }
  }

  /**
   * Download and replay backup data from R2 to restore company data.
   */
  private async importTableData(
    ...
    for (const table of COMPANY_TABLES) {
      ...
      try {
        ...
        if (error) {
          appLogger.error(`Failed to restore table ${table}`, error)
          continue
        }
        ...
        // Table data file may not exist in older backups
        continue
      }
    }

    return { tables: restoredTables.join(', '), rows: totalRows }
  }
