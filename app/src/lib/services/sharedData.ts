...
export class SharedDataService {
  ...
  async synchronizeData(
    ...
    await this.auditProjectUpdate(workItemId, conflicts, context, projectSnapshot, projectUpdateData)
    ...
    for (const docId of selectedDocumentIds) {
      try {
        ...
        if (docError) {
          ...
          await this.auditDocumentUpdate(docId, conflicts, context, snapshot.document_data, docData, docError.message)
        } else {
          documentResults.push({ documentId: docId, documentNumber: docId, success: true })
          updatedDocIds.push(docId)
          updatedCount++
          await this.auditDocumentUpdate(docId, conflicts, context, snapshot.document_data, docData)
        }
      } catch (err: any) {
        documentResults.push({ documentId: docId, documentNumber: docId, success: false, error: err?.message || 'Unknown error' })
        const snapshot = docSnapshots[docId]
        await this.auditDocumentUpdate(
          docId,
          conflicts,
          context,
  ...
  private async auditProjectUpdate(
    workItemId: string,
    conflicts: SharedDataConflict[],
    context: RequestContext,
    projectSnapshot: any,
