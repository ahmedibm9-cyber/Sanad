...
export class WorkItemService {
  ...
  async restoreWorkItem(id: string, context: RequestContext): Promise<WorkItem> {
    ...
    // Fetch the work item BEFORE restoring to get its type
    ...
    // Delete trash entry with correct entity_type
    const entityType = existing.type === 'project' ? 'project' : 'task'
    await (this.supabase as any)
      .from('trash_entries')
      .delete()
      .eq('entity_id', id)
  ...
  async getWorkItemMaterials(workItemId: string, context: RequestContext): Promise<WorkItemMaterial[]> {
    ...
    const { data, error } = await (this.supabase as any)
      .from('work_item_materials')
      .select('*, materials(name, grade, hs_code, origin, default_weight_unit)')
