...
class Logger {
  ...
  private async sendToRemote(entry: Record<string, unknown>): Promise<void> {
    // Send error/warn logs to a configurable telemetry endpoint
    // Set VITE_TELEMETRY_URL env var to enable (e.g., a Sentry DSN endpoint, webhook, or custom API)
    try {
      const telemetryUrl = (import.meta as any).env?.VITE_TELEMETRY_URL
      if (!telemetryUrl || (entry.level !== 'error' && entry.level !== 'warn')) return
