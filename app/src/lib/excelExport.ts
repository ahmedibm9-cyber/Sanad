...
function literalSpreadsheetValue(value: unknown): unknown {
...
export function exportToExcel(
  ...
  const safeRows = rows.map(row => row.map(literalSpreadsheetValue))
  const ws = XLSX.utils.aoa_to_sheet([headers.map(literalSpreadsheetValue), ...safeRows])
