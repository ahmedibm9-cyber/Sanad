...
export default function TrashPage() {
  ...
  const confirmRestoreAction = async () => {
    if (confirmRestore) {
      ...
      if (item) {
        const restored = await restoreEntry(item.entity_type, item.entity_id)
        if (!restored) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      {restoreError && (
        <div className="mx-6 mb-4 flex items-center gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
          <AlertTriangle size={16} />
          {t('Unable to restore this item. Please try again.', 'تعذر استعادة هذا العنصر. يرجى المحاولة مرة أخرى.')}
        </div>
      )}
