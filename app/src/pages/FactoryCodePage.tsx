...
export default function FactoryCodePage() {
  ...
  const { data, loading, error, refetch: refetchSearch } = useFactoryCodeSearch(debouncedSearch)
  const results = data ?? []
  const { data: allDataRaw, refetch: refetchAll } = useFactoryCodeAll()
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      FactoryCodeService.validateImportRows(rows)
      setParsedRows(rows)
  ...
  const handleApplyUpdate = async () => {
    ...
    try {
      ...
      const summary = await service.smartMergeWithStaging(parsedRows, requestContext, selectedFile || 'uploaded-file.xlsx')
      if (summary.errors > 0) {
        throw new Error(`Factory Code import failed validation for ${summary.errors} row(s); no success summary is available.`)
      }
