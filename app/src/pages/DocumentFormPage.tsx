...
const [docLanguage, setDocLanguage] = useState<'en' | 'ar'>(
  ...
  const [hasSignatureAsset, setHasSignatureAsset] = useState<boolean | null>(null)
  const [hasStampAsset, setHasStampAsset] = useState<boolean | null>(null)

  // Check if signature/stamp assets exist
  useEffect(() => {
    ...
    const checkAssets = async () => {
      ...
      const { data } = await (sb as any)
        ...
        .eq('active', true)
      const types = (data || []).map((a: any) => a.asset_type)
      setHasSignatureAsset(types.includes('signature'))
      setHasStampAsset(types.includes('stamp'))
    }
    checkAssets()
  }, [currentCompany?.id])
      ...
      <div className="flex gap-6 mt-4 pt-4 border-t border-gray-100">
        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
          <input
            ...
            onChange={e => {
              if (e.target.checked && hasSignatureAsset === false) {
                if (!confirm(t('No signature asset uploaded. The signature area will be empty. Continue?', 'لم يتم رفع ملف التوقيع. ستكون مساحة التوقيع فارغة. المتابعة؟'))) return
              }
          ...
          {hasSignatureAsset === false && <span className="text-amber-500 text-xs">({t('no asset', 'لا ملف')})</span>}
        </label>
        <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
          <input
            type="checkbox"
            checked={showStamp}
            onChange={e => {
              if (e.target.checked && hasStampAsset === false) {
                if (!confirm(t('No stamp asset uploaded. The stamp area will be empty. Continue?', 'لم يتم رفع ملف الختم. ستكون مساحة الختم فارغة. المتابعة؟'))) return
              }
          ...
          {hasStampAsset === false && <span className="text-amber-500 text-xs">({t('no asset', 'لا ملف')})</span>}
        </label>
      </div>
    </FormSection>
  )
