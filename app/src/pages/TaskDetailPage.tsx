...
export default function TaskDetailPage() {
  ...
  const { data: dbNotes = [], refetch: refetchNotes } = useNotes(id)
  const { data: dbIssues = [], refetch: refetchIssues } = useReportIssues(id)
  ...
  return (
    ...
    <AttachmentUploadModal
      ...
      onSave={async ({ file, category }) => {
        if (!user || !task) throw new Error(t('You must be signed in to upload files.', 'يجب تسجيل الدخول لرفع الملفات.'))
        const attachment = await attachmentService.uploadFile(file, task.id, category, {
          userId: user.id,
    ...
    <ProjectFormModal
      ...
      onSave={async (data) => {
        if (!task) return
        const result = await updateWorkItem(task.id, toWorkItemInput(data as Record<string, unknown>) as any)
        if (!result) throw new Error(t('The task could not be saved. Your entered data has been preserved.', 'تعذر حفظ المهمة. تم الاحتفاظ بالبيانات المدخلة.'))
      }}
      item={task}
      mode="task"
    />
    ...
    <ConfirmModal
      ...
      onClose={() => { setDeleteAttachmentId(null); setAttachmentActionError('') }}
      onConfirm={async () => {
      ...
      title={t('Move Attachment to Trash', 'نقل المرفق إلى سلة المحذوفات')}
      message={t(
        'The attachment will be moved to Trash and can be restored later.',
        'سيتم نقل المرفق إلى سلة المحذوفات ويمكن استعادته لاحقاً.'
      )}
      details={attachmentActionError || undefined}
      confirmLabel={t('Move to Trash', 'نقل إلى سلة المحذوفات')}
      cancelLabel={t('Cancel', 'إلغاء')}
      variant="danger"
