...
export default function ProjectFormModal({ open, onClose, onSave, item, mode }: ProjectFormModalProps) {
  ...
  const handleSave = async () => {
    ...
    setSaveError(null)
    try {
      await onSave({ ...item, ...form, materials: materialLines })
      onClose()
    } catch (error) {
      setSaveError(error instanceof Error ? error.message : t('The operation could not be saved. Your entered data has been preserved.', 'تعذر حفظ العملية. تم الاحتفاظ بالبيانات المدخلة.'))
    } finally {
      setSaving(false)
    }
  }
