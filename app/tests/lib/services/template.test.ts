...
describe('TemplateService', () => {
  ...
  describe('Template Definitions', () => {
    ...
    it('should have template-a, template-b, and all 7 native Fulla templates', () => {
      ...
      expect(keys).toContain('fulla-bill-of-lading-680')
    })
    ...
    it('uses generic descriptions for reusable templates', async () => {
      const { getAllTemplates } = await import('@/templates/registry')

      for (const template of getAllTemplates()) {
        expect(template.description).not.toContain('Fulla Trading Company')
        expect(template.descriptionAr).not.toContain('شركة فولا التجارية')
      }
    })
  })
